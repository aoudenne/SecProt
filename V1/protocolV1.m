...
--
-- 1. A -> B: {Kab}_Kb	--A sends B the secret key unencrypted
-- 2. B -> A: {Kab}_Ka	--B sends A the secret key back for confirmation
--
...
type
   ...
   MessageType: enum{
      M_KeyAddressU		--Kab:  Unencrypted Key
   };
   ...
   Message: record
      ...
      sender:	  AgentId;	 --purported sender of message
...
ruleset i:InitiatorId do
   choose j:net do
	 ...
	 inM: Message;	--incoming message
      ...
      begin
	 if ((inM.secretKey = ini[i].secretKey) & (inM.msgKey=i))then
...
ruleset i: ResponderId do
   choose j:net do
	 ...
	 inM:  Message;	   --incoming message
      ...
      begin
	 inM := net[j];
	 if ((inM.mType = M_KeyAddressU) & (inM.msgKey = i)) then
	    undefine outM;
	    outM.source	   := i;
	    outM.dest	   := inM.sender;
            outM.msgKey    := inM.sender;
   ...
   choose j: net do
	 ...
	 temp: Message;
      ...
      begin
	    ...
	    if(( msg.mType = M_KeyAddressU)  & (msg.msgKey = i)) then
	    ...
	    undefine temp.source;
            ...
            if multisetcount(l:messages, --add only if intruder has room
                ...
                messages[l].sender = temp.sender) = 0 then
