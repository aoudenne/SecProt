...
--
-- 1. A -> B: Kab	--A sends B the secret key unencrypted
-- 2. B -> A: Kab	--B sends A the secret key back for confirmation
--
...
ruleset i:InitiatorId do
   choose j:net do
	 ...
	 inM: Message;	--incoming message
      ...
      begin
	 if inM.secretKey = ini[i].secretKey then
	    ini[i].state := I_COMMIT;
	 else
	    --error "initiator received incorrect secret key"
	 end;
      end;
   end;
end;
...
ruleset i: ResponderId do
   choose j:net do
	 ...
	 inM:  Message;	   --incoming message
      ...
      begin
	 inM := net[j];
	 if inM.mType = M_KeyAddressU then
	    undefine outM;
	    outM.source	   := i;
...
ruleset i: IntruderId do
   choose j: net do
	 ...
	 alias msg: net[j] do	 --message that is intercepted
	    if msg.mType = M_KeyAddressU then
